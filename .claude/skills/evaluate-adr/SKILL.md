...
- データ収集（hooks）・バッチ処理・ダッシュボード・メトリクス設計に関わる**設計判断**
- 複数の選択肢を比較・検討したうえでの**意思決定**が含まれる
- 「なぜその設計を選んだか」が将来の変更時に参照価値を持つ
- データモデル・ストレージ形式・集計ロジックの変更
- 外部システム（Claude Code hooks、gh CLI 等）との連携方法の決定
...
- `TODO.md` に既に同一課題が記録されている

## 評価ステップ

### Step E-1: 既存 ADR・TODO との重複チェック

`$PWD/docs/adr/` を Glob で検索し、同一または類似の設計判断が既に記録されていないか確認する。`$PWD/TODO.md` も Read して同一課題がないか確認する。重複が見つかった場合は既存エントリを提示して処理を中断する。
...
- 重複課題 → 既存の ADR または TODO エントリを提示する

#### 要確認の場合の確認例

- 「この変更は設計判断（なぜこの方式を選ぶか）を伴いますか？それとも実装上の問題（バグ修正）ですか？」
