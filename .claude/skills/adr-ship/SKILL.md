...
description: This skill should be used when the user wants to "implement an ADR",
  "ADRを実装して", "ADR-055 を実装して", "ADR-055 を完了させて",
...
- ADR に定義された設計を実装し、ADR 内の受け入れ条件を自動検証する
- 条件未達の場合は修正して再検証することでサイクルを自動化する
- 完了時に ADR ステータスを更新してコミットする

## ステップ

### Step 1: ADR の特定

#### args が指定されている場合（例: `ADR-055`、`055`、`55`）
...
- ADR ファイルを Read し、`## コンテキスト`・`## 設計案` セクションの内容を把握する
- ADR ファイルの `## 受け入れ条件` セクションから `- [ ]` 行を抽出する

受け入れ条件が見つからない場合は AskUserQuestion でユーザーに確認し、必要に応じて ADR ファイルに追記するか中断するかを選択させる。
...
1. **「検証」セクション**: ADR の受け入れ条件を列挙し、各条件の検証方法（Glob/Read/Grep/Bash）を具体的に記述する
2. **「完了処理」セクション**: 検証通過後に ADR の `- [ ]` → `- [x]` 更新、ステータスの `採用済み` 更新、TODO.md から該当エントリの削除、CHANGELOG.md への記録、コミットを実行する旨を記述する
...
- **タスクが 4 件未満**: 複数の Edit / Write / Bash ツールを同一メッセージで並列呼び出しする
- **タスクが 4 件以上かつ独立**: Agent ツールで subagent を起動して並列実行する
...
1. ADR ファイルの `## 受け入れ条件` の `- [ ]` 行を1つずつ検証する
2. 検証方法は条件の文言に応じて決定する
   - ファイルの存在確認: Glob / Read
   - コードの内容確認: Grep / Read
   - 「動作する」「実行できる」「起動する」などの文言を含む条件: Bash でテスト実行する
     - ネットワーク通信を伴うコマンドは使用しない（curl, gh api 等）
...
1. ADR ファイルの `## 受け入れ条件` の `- [ ]` をすべて `- [x]` に更新する
2. ADR ファイルの `## ステータス` を `採用済み` に更新する
3. `$PWD/TODO.md` に該当 ADR のエントリがあれば削除する
4. `$PWD/CHANGELOG.md` に完了記録を追記する（日付 + ADR 番号 + タイトル）
...
- 操作対象は常に `$PWD` 配下
- ADR の `- [ ]` → `- [x]` 更新とステータス更新は Step 5 でのみ行う（Step 4 ループ中は変更しない）
