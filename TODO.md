...
- ADR-017: 設計/実装セッション分離の自動ディスパッチ
  ...
  - [x] `/dispatch` skill が TODO.md の未着手セクションから全タスクを検出できる
  - [x] `/dispatch` skill がタスクごとに worktree + tmux session + Claude Code を起動できる
  - [x] TODO.md の受け入れ条件を含む初期プロンプトが `tmux send-keys` で渡される（ADR 有無による分岐なし）
  - [x] `--dry-run` 引数で実際の起動なしに対象タスクを確認できる
  - [x] 既に worktree/ブランチが存在するタスクはスキップされる
